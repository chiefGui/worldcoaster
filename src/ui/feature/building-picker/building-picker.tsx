...
import { BuildingRegistry, type BuildingTypeId } from '@game/building/building.component'
...
export function BuildingPicker({ plotEntity, onClose }: BuildingPickerProps) {
  ...
  const handleSelect = (typeId: BuildingTypeId) => {
    if (!plotEntity) return
    BuildingAction.build({ plotEntity, typeId })
    onClose()
  }
        ...
        {buildings.map((building) => (
          ...
          >
            ...
            <div className="text-xs text-text-muted mt-1">
              ${building.inputAmount} · {building.capacity} capacity
            </div>
          </button>
        ))}
      </div>
